...
public class AcceptedFosterTribeRelationDecision : PolityDecision {
	...
	public AcceptedFosterTribeRelationDecision (Tribe sourceTribe, Tribe targetTribe) : base (sourceTribe) {
		...
		Description = "The leader of " + targetTribe.GetNameAndTypeStringBold () + ", " + targetTribe.CurrentLeader.Name.BoldText + ", has accepted the attempt from " + sourceTribe.GetNameAndTypeStringBold () + 
